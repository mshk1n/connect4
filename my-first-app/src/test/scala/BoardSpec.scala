...
class BoardSpec extends AnyWordSpec with Matchers {
  "A Board" should {
    ...
    "return false for out-of-bounds columns" in {
      val board = new Board()
      board.dropChip(-1, "X") should be(false)
      board.dropChip(7, "X") should be(false)
    }

    "correctly find the lowest empty row" in {
      val board = new Board()
      board.dropChip(0, "X") should be(true)
      board.getCell(5, 0) should be("X")
      board.dropChip(0, "O") should be(true)
    ...
    "return false if column is completely full" in {
      ...
      board.dropChip(0, "Y") should be(false)
    }
  }
}